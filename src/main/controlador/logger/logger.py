...
class LoggerJuego:
    ...
    def __init__(self,
        ...
        -
        'nombre': El nombre que tiene internamente el registrador.

        'nivel': El nivel por defecto que tiene el registrador.

        'formato': El formato que obedece al escribir información.

        'formato_fecha': El formato específico que sigue el prefijo de fecha.

        'verbose': Si imprimir por pantalla el log además de generar el archivo.
        """
        ...
        ruta_archivo = (self.ruta /
                        ahora.strftime("%Y%m%d-%H%M%S-%f")).with_suffix(LOG_EXT).as_posix()

        self.handler_archivos: FileHandler = FileHandler(filename=ruta_archivo, encoding='utf-8')
        self.handler_consola: StreamHandler = StreamHandler()
        self.actualizar_formateador()

        self.logger = Logger(name=nombre)
